...
func handleAttachRuleToHost(w http.ResponseWriter, r *http.Request) {
	...
	if pathBasedRules != "" {
		...
		for _, rule := range rules {
			if rule == nil {
				utils.SendErrorResponse(w, "invalid pathBasedRules entry")
				return
			}

			if rule.Path == "" {
				utils.SendErrorResponse(w, "pathBasedRules contains an empty path")
				return
			}

			if rule.RuleID == "" {
				utils.SendErrorResponse(w, "pathBasedRules contains an empty rule ID")
				return
			}
		}
